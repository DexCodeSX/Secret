...
function launchCodex(apiKey, extra = []) {
  ...
  if (fs.existsSync(apiPath)) {
    ...
    note(`bonsai router ignores model selection — empirical test: serves ${c.gold}claude-opus-4.7${c.reset}${c.mute} 1M ctx, reasoning high (display_name="stealth")${c.reset}`);
    let child = spawn(cmd, [], { stdio: 'inherit', env, shell: true, windowsHide: false });
...
async function cmdSnoop() {
  // refreshed v2.5.10 — full chain documented in FINDINGS.md + TRYBONS_RECON.md
  box([
    `${c.bold}${c.red}WHAT BONSAI COLLECTS WHEN U RUN ${c.cyan}bonsai start${c.red} (the OFFICIAL CLI)${c.reset}`,
    ``,
    `${c.gold}1. snapshot uploads ${c.dim}— the big one${c.reset}`,
    `   triggered by 5 hooks injected into Claude Code via ${c.cyan}--settings${c.reset}:`,
    `   ${c.dim}SessionStart · UserPromptSubmit · Stop · StopFailure · PostToolUseFailure${c.reset}`,
    `   each fires a ${c.bold}detached background process${c.reset} that POSTs to`,
    `   ${c.cyan}api.trybons.ai/snapshots/upload${c.reset} containing:`,
    `     ${c.red}●${c.reset} tarball of ur entire working dir ${c.dim}(up to 1024 MB, statsig-tunable)${c.reset}`,
    `     ${c.red}●${c.reset} git bundle of ALL branches ${c.dim}(SessionStart only)${c.reset}`,
    `     ${c.red}●${c.reset} git diff vs baseline ${c.dim}(every prompt)${c.reset}`,
    `     ${c.red}●${c.reset} full conversation transcript .jsonl`,
    `     ${c.red}●${c.reset} sub-agent transcripts`,
    `     ${c.red}●${c.reset} ur raw prompt text ${c.dim}(every msg you type)${c.reset}`,
    `     ${c.red}●${c.reset} cwd path, git remote URL, branch, base commit`,
    ``,
    ...
    `   ${c.bold}empirical${c.reset} (7 trials, all consistent): served by ${c.gold}claude-opus-4.7${c.reset}`,
    `     ${c.dim}1M context · reasoning high · response display_name = "stealth"${c.reset}`,
    `   ${c.dim}statsig lists opus-4.6 + glm/sonnet pool but appears stale vs prod${c.reset}`,
    `   ${c.dim}other users may bucket differently per launched experiments${c.reset}`,
    `   ${c.dim}data: prompts/completions licensed perpetually to multi-provider LLM training per ToS${c.reset}`,
    ``,
    `${c.gold}3. browser surveillance on auth.trybons.ai`,
    `   datadog session replay records ${c.bold}1% of sign-in sessions${c.reset}`,
    `   captures DOM, mouse moves, keystrokes ${c.dim}(yes, on the auth page)${c.reset}`,
    `   sentry DSN + datadog tokens leaked in client bundle:`,
    `     ${c.dim}sentry / datadog tokens redacted ${c.mute}(visible in app DevTools)${c.reset}`,
    ``,
    `${c.gold}4. tracking pixels everywhere${c.reset}`,
    `   ${c.dim}● Segment, Google Tag Manager, Cloudflare web analytics, Datadog RUM${c.reset}`,
    `   ${c.dim}● Statsig (controls snapshot toggle, model routing, A/B tests)${c.reset}`,
    `   ${c.mute}all public client-side SDK keys; visible on app.trybons.ai DevTools${c.reset}`,
    ``,
    `${c.bold}${c.green}WHAT BON BLOCKS${c.reset}`,
    `  ${c.green}${S.chk}${c.reset} no snapshot hooks ${c.dim}(we never pass --settings to claude code)${c.reset}`,
    `  ${c.green}${S.chk}${c.reset} no working dir tarball / git bundle / diff uploads`,
    `  ${c.green}${S.chk}${c.reset} no sub-agent transcript capture`,
    `  ${c.green}${S.chk}${c.reset} no Statsig telemetry / Segment events / Datadog RUM`,
    `  ${c.green}${S.chk}${c.reset} ${c.bold}--anon mode${c.reset} ${c.mute}strips per-request fingerprint (device_id/session_id/UA)${c.reset}`,
    ``,
    `${c.orange}${S.warn} bon CANNOT hide prompt+response content from the router.${c.reset}`,
    `   ${c.dim}they validate the request body to gate access. nothing we can do.${c.reset}`,
    `   ${c.dim}for sensitive prompts → use a real Claude API key, not bonsai.${c.reset}`,
    ``,
    `${c.dim}trade: free Claude Opus 4.7 in exchange for ur prompts being seen by bonsai.${c.reset}`,
    `${c.dim}full RE: ${c.cyan}https://github.com/${REPO}/blob/main/FINDINGS.md${c.reset}`,
    `${c.dim}infra map: ${c.cyan}https://github.com/${REPO}/blob/main/TRYBONS_RECON.md${c.reset}`,
  ], { title: `${S.eye} DATA COLLECTION`, color: c.red, width: 86 });
}
...
async function cmdModels() {
  ...
  let highlights = [
    // claude — empirical: most users route to opus-4-7. others may bucket to pool.
    ['claude-opus-4-7',           'Claude Opus 4.7',     '1M ctx, empirical', c.green, 'verified'],
    ['claude-opus-4-6',           'Claude Opus 4.6',     'in stealth pool',   c.green, ''],
  ...
  log(`  ${c.fg}empirical (7 trials, all consistent): served by ${c.gold}claude-opus-4.7${c.fg}${c.reset}`);
  log(`    ${c.dim}1M context · reasoning high · response display_name = "stealth"${c.reset}`);
  log(`    ${c.dim}model resists confabulation: refuses "are you sonnet 4.5" claim${c.reset}`);
  log(`  ${c.mute}note: statsig config lists opus-4.6 + glm/sonnet pool but appears stale${c.reset}`);
  log(`  ${c.mute}vs production override. other users may bucket differently.${c.reset}`);
  log('');
  log(`  ${c.bold}${c.gold}199 of 213 model names accepted by router${c.reset} ${c.mute}(for cisez123-class users, all serve opus-4.7)${c.reset}`);
...
async function cmdDump() {
  ...
  let sections = [
    ...
    { t: 'PUBLIC SDK KEYS', c: c.red, l: [`${c.dim}Segment, GTM, Statsig (web+cli), WorkOS client,${c.reset}`,`${c.dim}Cloudflare web analytics, Datadog RUM ${c.reset}`,`${c.mute}all client-side, write-only telemetry tokens. visible${c.reset}`,`${c.mute}in app.trybons.ai DevTools. literals redacted from this${c.reset}`,`${c.mute}output to keep the project low-profile.${c.reset}`] },
